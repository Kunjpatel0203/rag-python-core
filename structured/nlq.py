...
def load_dataframe(file_path):
    ...
    # Normalize column headers ONCE here
    df.columns = [normalize(c) for c in df.columns]
...
def get_column_samples(df, max_samples=5):
    samples = {}
    for col in df.columns:
        unique_vals = df[col].dropna().astype(str).unique()[:max_samples]
        samples[col] = unique_vals.tolist()
    return samples
...
Rules:
- Use semantic understanding, not exact string matching.
- If any sample value appears in the user question,
  you MUST use that column as filter_column and return lookup.
- DO NOT return not_applicable if a filter_value exists in samples.

Allowed operations:
lookup, sum, count, average, select_all, describe, not_applicable
...
# =====================================================
# Execute Avoided Here (executor handles execution)
# This file only loads + plans
# =====================================================
